...
from utility import to_numpy, get_result_dirs, get_result_paths,get_timing_responses
import config_file as c
from model_fitting import run_fit,compute_variance_explained
...
# from take_out_tuned import switch_off_tuned_nodes,switch_off_tuned_nodes_last_layer,switch_off_nodes_last_layer,assess_tuning_significance,switch_off_tuned_vs_mono_nodes_last_layer,relate_acc_tuned_peak
from parameter_stats import compare_maps_tuned_predictions#,compare_exponents_and_recurrentWeights
...
if __name__ == "__main__":
    ...
    if False:
        # train networks with diffeTrent depths with similar amount of parameters 
    ...
    # NOTE WHEN RUNNING THIS LOOP YOU WILL ALSO WANT TO CHANGE SETTINGS IN CONFIG_FILE
    if eval_controls == True:
        for condition in ["no_recurrency"]:#["init","shuffled","no_recurrency","16nodes"]:
            ind_rnn_bool = True
            shuffled_bool = False
            layers = 5
            num_hidden = 16
    ...
    # pickle_info_acc = do_accuracy_evaluations(results_dir, repetitions_all_depths, condition_list = condition_list,elem_idx = 117)
    # import pickle
    # with open('/media/evi/500GB/networks_rnn/VisualEventTimingGNN-master/VisualEventTimingGNN-master/stats_and_figs/accuracy/pkl_info_top_25_depths_1_2_3_4_5_conditions_16nodes_16nodes_16nodes_16nodes_16nodes.pkl', 'rb') as f:
    #     loaded_things = pickle.load(f)
    # repetitions_all_depths = list(loaded_things['state_change_repetitions_best_accuracies'].values())
    # pickle_info_1 = do_mono_tuned_fit_evaluations(results_dir,repetitions_all_depths,control_condition_list=condition_list,threshold =0.2,x0=["duration","ISI"],y0=["period","period"])
    pickle_info = compare_maps_tuned_predictions(results_dir,repetitions_all_depths,assessed_per="event",x0=["duration","ISI"],y0=["period","period"],control_condition_list=condition_list, threshold = 0.2,timing_types_separate=False,parameters_type = ["mono"])
    # fit_exp, rnn_hh_weight = compare_exponents_and_recurrentWeights(results_dir,repetitions_all_depths,assessed_per="event",x0=["duration","ISI"],y0=["period","period"],control_condition_list=condition_list, threshold = 0.2,timing_types_separate=False,parameters_type = ["tuned"])

# plt.hist(mean_acc_2_tuned, alpha=0.5)
# plt.hist(mean_acc_2_mono, alpha=0.5)
# plt.hist(mean_acc_2_nothing, alpha=0.5)
# plt.hist(mean_acc_2_random, alpha=0.5)
# plt.show

# plot_response_heatmaps(timing_res, timing_labels, neuron_idx=0,x0="duration",y0="period", normalize_per_plot=True,layer_id = None,split = 0)
