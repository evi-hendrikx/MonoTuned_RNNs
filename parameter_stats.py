...
import math
from scipy.stats import kruskal, shapiro,mannwhitneyu,wilcoxon,levene,skew, kurtosis
...
from utility import get_timing_responses, save_stats, get_result_paths,to_numpy
from model_fitting import tuned_function, mono_function, fit_responses,find_peak_tuning,refit_without_exponent
import config_file as c
from plotting import plot_response_heatmaps

def circ_percentiles(data, median, percentile_above_median):
    # median = 0 and all angles are between 0 and 2pi
    if percentile_above_median >0:
        transformed_data = np.mod(data-median+2*np.pi,2*np.pi)
    else:
        transformed_data = np.mod(median - data+2*np.pi,2*np.pi)
    ...
    n = len(sorted_data)
    print(n)
    k = (np.abs(percentile_above_median)/100) * (n - 1)
    print(k)
    ...
    print(c)
    if f == c:
        centered_percentile = sorted_data[f]
    else:
        d0 = sorted_data[f] * (c - k)
        d1 = sorted_data[c] * (k-f)
        centered_percentile = d0 + d1
        
    print(centered_percentile)
        
    # adjust back to normal space
    if percentile_above_median > 0:
        percentile_value = centered_percentile + median
    else:
        percentile_value = median - centered_percentile
    # check what happens if you use negative percentiles
    return percentile_value

def distances_peaks_stats(distances_peaks, distances = True,variable='distances',parameters_type=[],name_dict={}):
    ...
    plot_info = []
    for network_depth in distances_peaks.keys():
        ...
        for key in layer_order:
            if key in list(distances_peaks[network_depth].keys()):
                eval_string = eval_string + "distances_peaks[" + str(network_depth) + "][" + str(key) + "],"
                for node in distances_peaks[network_depth][key]:
        ...
        for key_id, key in enumerate(layer_order):
            ...
            for key2 in layer_order[key_id+1:]:
                if key in list(distances_peaks[network_depth].keys()) and key2 in list(distances_peaks[network_depth].keys()) :
                    ...
                    if variable != "fit_theta":
                        ...
                        if variable != "fit_betaRatio":
                            eval_string = "levene(distances_peaks[" + str(network_depth) + "][" + str(key) + "]," + "distances_peaks[" + str(network_depth) + "][" + str(key2) + "],center = 'median')"
                        ...
                        eval_string = "circstat.tests.watson_williams(distances_peaks[" + str(network_depth) + "][" + str(key) + "]," + "distances_peaks[" + str(network_depth) + "][" + str(key2) + "])"
                        ...
                        eval_string = "circstat.tests.cmtest(distances_peaks[" + str(network_depth) + "][" + str(key) + "]," + "distances_peaks[" + str(network_depth) + "][" + str(key2) + "])"
        ...
        print(network_depth)
        # print(stats['between_layers'][network_depth]["post_hoc"]['print'])
        # print(stats['between_layers'][network_depth][posthoc_name_field]["posthoc_corrected"])
        # print(stats['between_layers'][network_depth]["variances"])
    ...
    if distances == True:
        ...
        for network_depth in distances_peaks.keys():
            ...
            first_layer = distances_peaks[network_depth][0]
            all_first_layers.append(first_layer)
            last_layer = distances_peaks[network_depth][network_depth-1]
    ...
    if "tuned" in parameters_type:
        for var in ['fit_x0', 'fit_y0','fit_sigmaMajor','fit_sigmaMinor','fit_theta','fit_ratio','fit_exp']:
            ...
            if control_condition_list != ['no_recurrency']:
                distances_peaks_stats(eval(var), distances = False,variable=var,parameters_type=parameters_type,name_dict=name_dict)
...
# def compare_exponents_and_recurrentWeights(results_dir,all_repetitions,assessed_per="event",x0=["duration","ISI"],y0=["period","period"],control_condition_list=[[None]], threshold = 0.2,timing_types_separate= True,parameters_type = ["tuned","mixed","mono"]):
...
#     total_df = load_fit_data(results_dir, all_repetitions,assessed_per,x0,y0,control_condition_list, threshold,to_assess = ['exponent','node'])
    
#     #TODO: get recurrent weights for each node

#     for response_type in ["tuned","mixed"]:    
#         for timing_type in ['_' + x0[timing_type_id] + '_'+ y0[timing_type_id] for timing_type_id, _ in enumerate(x0)]:
...
#                     # recheck position peaks
#                     if response_type in parameters_type:
#                         repetition = {"num_layers": row['depth'], "num_hidden": num_hidden, "norm": "layer_norm", "batch_size": 50, "ind_rnn": True, "weight_constraint": False,'LR':2e-3,'dropout_prob': c.DROPOUT_PROB,'assessed_per':'event','cross_v': True,'weight_reg':1e-08, 'scrambled_control': False, 'counter': row['rep']}
...
#                         string_get_weights = 'rnn.layers.' + str(row['layer']) + '.rnn_cell.weight_hh'
#                         # print(to_numpy(network_weights[string_get_weights]))
#                         # print(row['node'])
#                         # print(node_id)
                        
 
#                         if not row['depth'] in fit_exp.keys():
#                             fit_exp[row['depth']] = {}
#                             rnn_hh_weight[row['depth']] = {}
#                         if not row['layer'] in fit_exp[row['depth']].keys():
#                             fit_exp[row['depth']][row['layer']] = []
#                             rnn_hh_weight[row['depth']][row['layer']] = []

#                         fit_exp[row['depth']][row['layer']].append(float(row['exponent'][node_id]))
#                         rnn_hh_weight[row['depth']][row['layer']].append(to_numpy(network_weights[string_get_weights])[int(row['node'][node_id])])

                        
#                         # string_in_weights = 'rnn.layers.' + str(int(row['layer'][node_id])) + '.rnn_cell.weight_ih'
#                         # to_numpy(network_weights[string_get_weights])[:][int(row['node'][node_id])]

#                         # print(rnn_hh_weight)

#                         # print(float(row['exponent'][node_id]))
                        
                        
#     return fit_exp, rnn_hh_weight

def bimodality_coefficient(data):
    n = sum(np.isnan(data))
    skewness = skew(data)
    kurt = kurtosis(data,fisher = False)# TODO: check what fisher thing does
