...
def tuned_function(response_labels,  x0, y0, sigma_x0, sigma_y0, theta, exponent, slope, intercept):
    warnings.simplefilter('ignore', lineno=54)
...
def mono_function(response_labels,  x0, y0, ratio_x0, ratio_y0,slope,intercept):
    warnings.simplefilter('ignore', lineno=66)
...
# def tuned_no_exp_function(response_labels,  x0, y0, sigma_x0, sigma_y0, theta, slope,intercept):
...
#     # periods are usually not negative,
#     # but in case of testing negative periods: Numpy does not seem to allow 
#     # fractional powers of negative numbers, even if the power would not result in a complex number
#     r =  ((np.exp(power)) * slope) + intercept

#     return r


def refit_without_exponent(prediction, timings,initial_guess_x, initial_guess_y,tuned_params):
    ...
    while attempt <= 100:
        ...
        if solved == False:
            ftol = 1e-4
            initial_guess = [initial_guess_x,initial_guess_y,*tuned_params[2:5],*tuned_params[6:]]
            attempt += 1
        else:
            ftol = ftol/10
            initial_guess = prediction_fit[0]
    
        try:

            # put in bounds for slope --> else can go negative, then sigmas etc also negative
            # put bound
            function_to_fit = tuned_no_exp_function
            prediction_fit = curve_fit(
                function_to_fit, 
                timings, 
                prediction, 
                p0 = initial_guess, 
                bounds = ((-np.inf, -np.inf, 0.001, 0.001,0,1e-8,-np.inf), (np.inf, np.inf,10, 10, 180,np.inf, np.inf)),
            ...
            # if it could not be solved with large stepsize after 100 random initializations: give up
                ...
                print(f'optimal parameters not found after max evaluations. {attempt}/100 attempts')
                if attempt > 10:
                    # start from a different initial guess
                    prediction_fit = [(c.FIT_SETTINGS["initial"]['tuned'][1] - c.FIT_SETTINGS["initial"]['tuned'][0]) * np.random.random(c.FIT_SETTINGS["initial"]['tuned'][0].shape) + c.FIT_SETTINGS["initial"]['tuned'][0]]
                if attempt == 100:
...
def compute_variance_explained(function_type, node_activity, response_labels, params):
    ...
    if function_type == "tuned":
        ...
        function_to_use = tuned_no_exp_function
...
def run_fit(repetitions, assessed_per="event",x0="duration",y0="period",control_condition=None):
    ...
    if not os.path.exists(event_response_path) or not os.path.exists(state_change_response_path) or not os.path.exists(movie_response_path) or not os.path.exists(response_labels_path):
        # ValueError("????? it says it doesn't have the path")
