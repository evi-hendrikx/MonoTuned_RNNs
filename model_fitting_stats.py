...
def load_fit_data(results_dir,repetitions,assessed_per="event",x0=["duration","ISI"],y0=["period","period"],control_condition_list=[], threshold = 0.2,to_assess = "prop"):
    '''
            ...
            for layer in range(amount_layers + 1):
                ...
                for row_id, winner in enumerate(highest_VE):
                    ...
                    if "tuned" in winner and above_threshold:
                        ...
                        # out_range = np.logical_or(np.logical_or(np.logical_or(tuned_param_values[tuned_params[timing_space].index('pref_x0'+timing_space)]>0.95,tuned_param_values[tuned_params[timing_space].index('pref_x0'+timing_space)]<0.05),
                        #     np.logical_or(tuned_param_values[tuned_params[timing_space].index('pref_y0'+timing_space)]>1,tuned_param_values[tuned_params[timing_space].index('pref_y0'+timing_space)]<0.1)),
                        #     tuned_param_values[tuned_params[timing_space].index('pref_y0'+timing_space)]<tuned_param_values[tuned_params[timing_space].index('pref_x0'+timing_space)] + 0.05)
                        ...
                        if invalid_tuned[timing_space][row_id] == False and out_range == False:
                            ...
                            # plt.scatter(combis_x, combis_y,c=tuned_prediction)
                            # plt.show()
                        else:
                ...
                for model_name in ["mono","tuned","mixed"]:
                    ...
                    if to_assess != None and to_assess != ["prop"] and to_assess != ["ve"]:
                       if to_assess != ["classifications"]:
                        ...
                        # try:
                        #     prop_arr.append(sum(eval(classified_string))/(sum(mono_classified) + sum(mixed_classified) + sum(tuned_classified)))
                        # except:
                        #     prop_arr.append(0)
...
def kruskal_and_posthocs(total_df, dependent_var, between_what, additional_factors = {}):
...
def stats_fits_between_depths(results_dir,repetitions,assessed_per="event",x0="duration",y0="period",control_condition_list=[], threshold = 0.2, model_list = ["mono","mixed","tuned"], last_layer = False, to_assess= "prop",first_layer= False):
    '''
...
def paired_compare_conditions(results_dir,repetitions,assessed_per="event",x0="duration",y0="period",control_condition_list=[], threshold = 0.2,to_assess = None):
    '''
    Stats and plots between 
    ...
    '''
    # proportions
    total_df = load_fit_data(results_dir, repetitions,assessed_per,x0,y0,control_condition_list, threshold,to_assess)
    
    save_stats = {}
    save_stats['prop']={}
    for model in ["mono","tuned"]:
        list_tuples = []

        stats = kruskal_and_posthocs(total_df,'prop', 'condition',{'condition':['regular','shuffled'],'depth':[5],'model':[model],'layer':["overall"]})
        save_stats['prop'][model] = stats
        ...
        if model == "mono":
              ...
              plot_df['prop'] = plot_df['prop'].astype(float)
        elif model == "tuned":
            new_df = pd.DataFrame.from_records(list_tuples,columns=[stats['input_args']['independent_var'], *list(stats['input_args']['selection'].keys()),stats['input_args']['dependent_var']])
            new_df = new_df.T.drop_duplicates().T   
            new_df['prop'] = new_df['prop'].astype(float)
            plot_df = pd.concat([plot_df, new_df])

    plot_violins(plot_df, stats,hue="model")
    
    # VE
    total_df_stretched_ve = total_df.explode('ve')
    save_stats['ve']={}
    for model in ["mono","tuned"]:
        list_tuples = []

        # for layer in range(5):
        stats = kruskal_and_posthocs(total_df_stretched_ve,'ve', 'condition',{'condition':['regular','shuffled'],'depth':[5],'model':[model],'layer':["overall"]})
        save_stats['ve'][model] = stats
        ...
        if model == "mono":
              plot_df = pd.DataFrame.from_records(list_tuples,columns=[stats['input_args']['independent_var'], *list(stats['input_args']['selection'].keys()),stats['input_args']['dependent_var']])
              plot_df = plot_df.T.drop_duplicates().T   
              plot_df['ve'] = plot_df['ve'].astype(float)
        elif model == "tuned":
            new_df = pd.DataFrame.from_records(list_tuples,columns=[stats['input_args']['independent_var'], *list(stats['input_args']['selection'].keys()),stats['input_args']['dependent_var']])
            new_df = new_df.T.drop_duplicates().T   
            new_df['ve'] = new_df['ve'].astype(float)
            plot_df = pd.concat([plot_df, new_df])

    plot_violins(plot_df, stats,hue="model")
    
def stats_fits_between_layers(results_dir,repetitions,assessed_per="event",x0="duration",y0="period",control_condition_list=[], threshold = 0.2, to_assess = "prop",model_list = ["mono","mixed","tuned"]):
    '''
    Stats and plots between different layers of a network
   
    Seems that kruskal wallis is fairly fine with unequal sample sizes (more than ANOVA at least)
...
def do_mono_tuned_fit_evaluations(results_dir,repetitions_all_depths,assessed_per="event",x0=["duration","ISI"],y0=["period","period"],control_condition_list=[], threshold = 0.2):
    ...
    # stats_fits_between_depths(results_dir,repetitions_all_depths,assessed_per,x0,y0,control_condition_list, threshold,last_layer=False)
    # stats_fits_between_depths(results_dir,repetitions_all_depths,assessed_per,x0,y0,control_condition_list, threshold,last_layer=True)
    # stats_fits_between_depths(results_dir,repetitions_all_depths,assessed_per,x0,y0,control_condition_list, threshold,to_assess='ve',first_layer = True)


    pickle_info = stats_fits_between_layers(results_dir,repetitions_all_depths,assessed_per,x0,y0,control_condition_list,threshold)
    return pickle_info
