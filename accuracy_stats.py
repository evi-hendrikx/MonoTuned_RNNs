...
def plot_performance_heatmap_size_std(performance_measure, labels, stds=[], do_plotting = True,ax = None):
    not_show = False
...
def compare_accuracy_model_depths(results_dir, all_repetitions, condition_list = [],top = 50):
    ...
    for layer_id,repetitions_per_layer in enumerate(all_repetitions):
        ...
        for repetition in repetitions_per_layer:
            ...
            if control_condition == "special_condition":
                evaluate_special_conditions(results_dir, repetition, assessed_per="event",x0="duration",y0="period", do_plotting = False,control_condition=control_condition)
            else:
...
def compare_variance_accuracy_maps(results_dir, all_repetitions, assessed_per="event",x0="duration",y0="period",condition_list = [],top = 50):
    '''
    ...
    for network_depth_id, repetitions_per_layer in enumerate(all_repetitions):      
        ...
        for repetition in repetitions_per_layer:
            ...
            if control_condition == "special_condition":
                ...
                evaluate_net(results_dir, repetition, assessed_per,x0,y0,do_plotting = False,control_condition=control_condition)
            _, test_results_path, _, _, _, _, _, _ = get_result_paths(results_dir, repetition,control_condition=control_condition) 
            with open(test_results_path, 'rb') as f:
                # get concatenated losses and fit_labels
...
def do_accuracy_evaluations(results_dir, repetitions_all_depths, elem_idx=117, condition_list = []):
    
    # select relevant repetitions to plot loss
    normal_ids = np.where(np.array(condition_list) == "no_recurrency")[0]
...
#    select (other) relevant repetitions to plot loss
    ...
    # plot_loss_functions_all_depths(results_dir, repetitions_5_layers,control_condition_list = condition_list_5_layers)
    ...
    # repetitions_5_layers = [repetitions_all_depths[i] for i in normal_id_5_layer]
    # condition_list_5_layers = [condition_list[i] for i in normal_id_5_layer]
    # # compare_accuracy_model_depths(results_dir, repetitions_5_layers, condition_list = condition_list_5_layers)
